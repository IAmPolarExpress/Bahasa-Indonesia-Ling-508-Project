...
def test_lexical_entry():
    ...
    le = LexicalEntry(written_form="untuk", origin=4)
    assert le.written_form == "untuk"
    assert le.origin == OriginLanguage.MALAY

    le2 = LexicalEntry(written_form="emoji", origin=2)
...
def test_make_sense():
    ...
    sense1 = Sense(written_form="advocat", origin=1, pos=1, definition="lawyer")
...
def test_spoken_word():
    ...
    spokenword1 = SpokenWord(written_form="sedikit", origin=4, pos=3, definition="little", es_in_word=1, \
